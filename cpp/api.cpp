		...
		int32_t* match_length, int32_t* prefix_length)
{
	...
	*prefix_length = 0;
	if (utf8_prefix[0] != '\0' && search_mode == DEFAULT_DICT && config.default_dictionary == "words") {
		std::string extended_text = crf_extend(utf8_text, utf8_prefix, prefix_length);
		if (extended_text.size() > 0)
		{
			SearchResult res2 = search(extended_text.c_str(), SearchMode(search_mode));
			if (res2.max_match_symbols_length - *prefix_length >= res.max_match_symbols_length)
			{
				res = res2;
			}
			else
	...
	*match_length = int32_t(res.max_match_symbols_length);
